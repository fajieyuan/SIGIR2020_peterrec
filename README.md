...
(LambdaFM: Learning Optimal Ranking with Factorization Machines Using Lambda Surrogates) or NCE loss used in word2vec. Similarly, our pretraining network (e.g., NextitNet_TF_Pretrain.py) also employs a basic sampling function in TF, you can also replace it with your own one if you are dealing with hundreds of millions of item pool in a large-scale system.
